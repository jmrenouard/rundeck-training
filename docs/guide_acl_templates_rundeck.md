...
by:
  group: readonly_all
Note importante : L'action read sur les jobs permet de voir la définition complète du job, y compris le workflow. Si tu veux permettre de voir l'existence du job sans sa définition, utilise uniquement view au lieu de read.​
...
by:
  group: executors_projectx
Variante pour un groupe de jobs spécifique : Si tu veux limiter l'exécution à un dossier de jobs uniquement (par exemple /prod/maintenance), ajoute une règle match sur le groupe de jobs:​
...
PagerDuty Runbook Automation (Enterprise/SaaS):​

ACL Policy Editor avec wizard graphique : interface guidée pour créer des règles sans écrire de YAML manuellement

ACL Storage Layer optimisé (depuis v3.4.0) : stockage des ACL dans la base de données pour de meilleures performances avec de nombreuses ACL​

Test ACL via GUI : possibilité de tester les permissions directement dans l'interface pour un utilisateur ou groupe donné​

ACL granulaires pour Key Storage par projet (depuis v3.4.1) : isolation complète des secrets au niveau projet au lieu du niveau système​

Délégation de l'administration des ACL projet : les administrateurs peuvent déléguer la gestion des ACL projet aux admins de projet​

Configuration via GUI : les paramètres sont stockés en base de données et peuvent être partagés entre les membres d'un cluster​

Version SaaS : gestion uniquement via GUI, pas d'accès filesystem aux fichiers .aclpolicy​
...
Sépare tes ACL par groupe ou par usage : Au lieu d'un seul fichier monolithique, crée plusieurs fichiers .aclpolicy :​
...
Pour correspondances exactes (éviter l'évaluation regex), utilise le format URN:​
...
Pour plusieurs projets similaires, utilise les regex intelligemment:​

text
context:
  project: '(dev|test|qa)_.*'  # Tous les projets dev_, test_, qa_
Hiérarchie Allow/Deny
Le deny l'emporte toujours sur allow. Si une règle autorise une action mais qu'une autre la refuse, l'action sera refusée. Utilise deny avec précaution et de préférence avec notBy pour exclure certains utilisateurs:​
...
Valide la syntaxe YAML avec rd-acl validate <fichier.aclpolicy>

Teste avec rd-acl test pour simuler les permissions

Consulte /var/log/rundeck/rundeck.audit.log pour voir les décisions d'autorisation GRANTED/REJECTED/DENIED

Active le mode debug temporairement pour comprendre les évaluations de règles

Dans PagerDuty Runbook Enterprise, utilise l'évaluateur ACL intégré dans le GUI pour tester les permissions d'un utilisateur ou groupe avant déploiement.​

Délégation Progressive
Commence par des permissions restrictives et ajoute progressivement :​

Commence avec lecture seule sur un projet

Ajoute run sur jobs spécifiques

Ajoute kill pour gérer les exécutions

Enfin, ajoute update/create si nécessaire

Cette approche limite les risques et suit le principe du moindre privilège.

Gestion du Key Storage
Pour les projets nécessitant des clés SSH ou secrets :​
...
Vérifie que l'utilisateur appartient bien au groupe (page profil utilisateur)

Vérifie qu'il y a à la fois un contexte application ET project défini

Consulte rundeck.audit.log pour voir la dernière décision (GRANTED/REJECTED/DENIED)

Vérifie qu'il n'y a pas de règle deny qui s'applique

Pour accéder à un projet, l'utilisateur doit avoir read sur le projet dans le contexte application​
